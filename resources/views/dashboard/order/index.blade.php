                    ...
                    <div class="m-portlet__head-caption">
                        <div class="m-portlet__head-title">
                            <h3 class="m-portlet__head-text">
                                <a href="{{url('/admin/orders/create')}}" class="btn btn-success" {{--data-toggle="modal" data-target="#m_modal_6"--}}><i class="fa fa-plus pr-2"></i>اضافة دورة</a>
                            </h3>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="col-md-12" id="ajax">
                            <table id="datatable" class="text-center table table-bordered" cellspacing="0" width="100%">
                                ...
                                @foreach($orders as $key => $order)
                                    <tr>
                                        <td>{{++$key}}</td>
                                        <td><a href="{{url('/admin/clients/'.$order->client['id'].'/edit')}}">{{$order->client['name']}}</a></td>
                                        <td>{{$order->title}}</td>
                                        <td>{{$order->email}}</td>
                                        <td>{{$order->service}}</td>
                                        <td>{{$order->budget}}</td>
                                                        ...
                                                        <div class="m-dropdown__body">
                                                            <div class="m-dropdown__content">
                                                                <ul class="m-nav">
                                                                    ...
                                                                    @can('delete orders')
                                                                        <li class="m-nav__item">
                                                                            <form action="{{route('orders.destroy',$order->id)}}" class="m-nav__link  lh"  method="post">
                                                                                @csrf @method('DELETE')
